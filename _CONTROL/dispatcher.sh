...
7. Belege jede Sachbehauptung mit Fußnoten, trage die Datei mit Audit-Modus in _CONTROL/AUDIT-QUEUE.md ein und starte den Subagenten brainvault-auditor mit run_in_background: true. Mache danach sofort mit der nächsten Kanal-Nachricht weiter — der Auditor läuft parallel. Der Auditor schreibt nur in die Note selbst (Frontmatter); LOG.md, TASKS.md und AUDIT-QUEUE.md aktualisierst du nach Rückmeldung des Subagenten.
